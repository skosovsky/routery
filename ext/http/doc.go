// Package routeryhttp adapts net/http clients to routery handlers.
//
// The adapter returns [routery.RouteResult] values with *http.Response payloads for
// successful (2xx) calls. For non-2xx responses it returns *StatusError; inspect the
// response body via StatusError.Response while using routery.RetryIf with DefaultRetryPolicy.
//
// NewHandler makes request bodies replayable. When a request has Body but no
...
// cancels the context as soon as Handle returns, but HTTP response bodies are
// often read after headers have been returned. Timeout ties cancellation to
// Response.Body.Close.
//
// Middlewares between routery.RetryIf and NewHandler must not clone
// *http.Request with Clone or WithContext. NewHandler mutates the original
...
// must not be placed between RetryIf and NewHandler.
package routeryhttp
